...
export async function PATCH(request:Request,{params}:{params:Promise<{id:string}>}){
 ...
 if(typeof name!=="string"||!name.trim()||!Number.isInteger(price)||price<0||typeof category!=="string"||!category.trim())return NextResponse.json({error:"Invalid menu item"},{status:400});
 const[result]=await sql`UPDATE menu_items SET name=${name.trim()},description=${description||""},price_cents=${price},category=${category.trim()},image=${image||""},available=${!!available},sort_order=${sort},updated_at=NOW() WHERE id=${id} RETURNING *`;return NextResponse.json({item:result});
 }catch{return NextResponse.json({error:"Unable to update menu item"},{status:500})}
}
