...
export async function POST(request: Request) {
  try {
    ...
    if (!customer?.name || !customer?.phone || !customer?.orderDate || !customer?.preferredTime || !customer?.fulfillment || !Array.isArray(items) || items.length === 0) {
      return NextResponse.json({ error: "Missing required order details" }, { status: 400 });
    }
    if (!items.every((item: any) => item?.id && item?.name && Number.isInteger(item?.quantity) && item.quantity > 0 && Number.isFinite(item?.price) && item.price >= 0)) {
